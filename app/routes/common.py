...
# =============================================================================
# ENDPOINT: Capacidades de Elementos (Genérico)
# =============================================================================

@bp.route('/element-section-diagram', methods=['POST'])
@handle_errors
@require_session
def get_element_section_diagram(session_id: str, data: dict):
    """
    Genera diagrama de sección transversal de cualquier elemento.

    Request JSON:
        {
            "session_id": "...",
            "element_key": "Story_Label",
            "element_type": "pier" | "column",
            "proposed_config": {...}  // opcional, solo para pier
        }
    ...
    if not element_key:
        return jsonify({'success': False, 'error': 'Se requiere element_key'}), 400

    service = get_element_details_service()
    result = service.get_section_diagram(
